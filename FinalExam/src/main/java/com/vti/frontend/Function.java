...
public class Function {
	...
	public void getProjectInfo() throws ClassNotFoundException, SQLException {
		
		while (true) {
			System.out.print("Mời bạn nhập vào id của project: ");
			int projectID = ScannerUtils.inputPositiveInt("Bạn phải nhập vào số nguyên dương!");

			if (userController.isProjectIdExits(projectID)) {
				List<User> users = userController.getProjectInfo(projectID);
				System.out.printf("%-15s %-25s %-25s %-25s %-25s\n", "ID", "Email", "Fullname", "Position" ,"Password");
				for (User user : users) {
	...
	public void getProjectManager() throws ClassNotFoundException, SQLException {
		List<User> users = userController.getProjectManager();
		System.out.printf("%-15s %-25s %-25s %-25s %-25s\n", "ID", "Email", "Fullname", "Position" ,"Password");
		for (User user : users) {
			System.out.printf("%-15s %-25s %-25s %-25s %-25s\n", user.getId(), user.getEmail(), user.getFullName(),user.getRole(),
					"*********");
